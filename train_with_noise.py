...
class SequenceDataset(Dataset):
    def __init__(self, sequences, labels, symbol_to_idx, omega_idx, noise_level=0.0, 
                 noise_type='replacement', alphabet=None, append_omega=True):
        ...
        self.append_omega = append_omega
    ...
    def __getitem__(self, idx):
        ...
        if self.append_omega:
            indices.append(self.omega_idx)
        
        return torch.tensor(indices, dtype=torch.long), torch.tensor(self.labels[idx], dtype=torch.float)


def _looks_like_poker_sequence(tokens):
    if not isinstance(tokens, list) or len(tokens) != 5:
        return False
    for t in tokens:
        if not isinstance(t, str):
            return False
        # Expected form: S<1-4>R<1-13>
        if not (t.startswith("S") and "R" in t):
            return False
        s_part, r_part = t[1:].split("R", 1)
        if not (s_part.isdigit() and r_part.isdigit()):
            return False
        s = int(s_part)
        r = int(r_part)
        if s < 1 or s > 4 or r < 1 or r > 13:
            return False
    return True


def _looks_like_parentheses_tokens(tokens):
...
def _paren_features(tokens):
    ...
    for t in tokens:
        ...
        if bal > max_bal:
            max_bal = bal
    return bal, min_bal, max_bal, trace


def _parentheses_to_leftmost_marked_token(tokens):
    # Human-readable + leftmost-attention-friendly: a single token that encodes
    # the full information needed for bracket-balance (sum + min-prefix).
    bal, min_bal, max_bal, trace = _paren_features(tokens)
    n = len(tokens)
    trace_str = ",".join(map(str, trace))
    # Mark "when balance appears" via the running prefix-balance trace.
    return f"LM|len={n}|trace={trace_str}|sum={bal}|min={min_bal}|max={max_bal}"


def _poker_hand_to_feature_token(tokens):
    # Human-readable + compact deterministic token.
    # Encode as rank multiplicities + suit histogram (order-invariant, much smaller vocab than raw hands).
    rank_counts = [0] * 13
    suit_counts = [0] * 4
    for t in tokens:
        s_part, r_part = t[1:].split("R", 1)
    ...
    ranks_part = ",".join([f"{rank_names[i]}x{c}" for i, c in enumerate(rank_counts) if c > 0])
    suits_part = ",".join([f"S{i+1}:{c}" for i, c in enumerate(suit_counts)])
    # Marked for leftmost-attention friendliness: the entire sequence repeats this token,
    # so the model's leftmost tie-breaking effectively "routes" information consistently.
    return f"LM|R[{ranks_part}]_S[{suits_part}]"


def load_data(csv_path, json_path=None, poker_repeat_len=6, export_adapted_csv=None):
    ...
    for _, row in df.iterrows():
        ...
        if "," in seq_str:
            ...
            # e.g. balanced parentheses datasets store sequences like "()()(()"
            seq = list(seq_str.strip())
        sequences.append(seq)
        labels.append(int(row['label']))

    append_omega = True
    # Balanced parentheses adaptation.
    if formula_data is None and len(sequences) > 0 and _looks_like_parentheses_tokens(sequences[0]):
        feature_seqs = []
        adapted_tokens = []
        for seq in sequences:
            tok = _parentheses_to_leftmost_marked_token(seq)
            adapted_tokens.append(tok)
            feature_seqs.append([tok] * poker_repeat_len)
        sequences = feature_seqs
        append_omega = False

        if export_adapted_csv:
            out_df = df.copy()
            out_df["adapted_token"] = adapted_tokens
            out_df.to_csv(export_adapted_csv, index=False)

    # Poker adaptation: featurize 5-card hands into a stable, trainable token sequence.
    if formula_data is None and len(sequences) > 0 and _looks_like_poker_sequence(sequences[0]):
        feature_seqs = []
        adapted_tokens = []
        for seq in sequences:
            tok = _poker_hand_to_feature_token(seq)
            adapted_tokens.append(tok)
            feature_seqs.append([tok] * poker_repeat_len)
        sequences = feature_seqs
        append_omega = False
    ...
    if formula_data is not None:
        ...
        # Infer alphabet directly from dataset (e.g. poker tokens like "S1R6")
        unique_tokens = set()
        for seq in sequences:
            unique_tokens.update(seq)
        alphabet = sorted(unique_tokens)

    symbol_to_idx = {symbol: idx for idx, symbol in enumerate(alphabet)}
    omega_idx = len(alphabet)
    vocab_size = len(alphabet) + 1
    
    return sequences, labels, symbol_to_idx, omega_idx, vocab_size, formula_data, alphabet, append_omega
...
def evaluate(model, dataloader, device, threshold=0.5):
    ...
    with torch.no_grad():
        for sequences, labels in dataloader:
            ...
            logits = model.forward_raw(sequences)
            probs = torch.sigmoid(logits)
            preds = (probs >= threshold).to(torch.long)
            all_preds.extend(preds.cpu().numpy().tolist())
            all_probs.extend(probs.cpu().numpy().tolist())
            all_labels.extend(labels.to(torch.long).cpu().numpy().tolist())
...
def train_model(sequences, labels, symbol_to_idx, omega_idx, alphabet, vocab_size,
                ...
                early_stop_patience=5, min_delta=0.001, append_omega=True):
    ...
    train_dataset = SequenceDataset(train_sequences, train_labels, symbol_to_idx, omega_idx, 
                                   noise_level, noise_type, alphabet, append_omega=append_omega)
    val_dataset = SequenceDataset(val_sequences, val_labels, symbol_to_idx, omega_idx, 0.0, 'replacement', alphabet, append_omega=append_omega)
    test_dataset = SequenceDataset(test_sequences, test_labels, symbol_to_idx, omega_idx, 0.0, 'replacement', alphabet, append_omega=append_omega)
...
def main():
    ...
    parser.add_argument('--csv_path', type=str, default='datasets_poker/poker_binary.csv',
                        help='Path to dataset CSV file')
    parser.add_argument('--json_path', type=str, default=None,
                        help='Optional path to formula JSON file (if omitted, alphabet is inferred from CSV)')
    parser.add_argument('--noise_levels', type=float, nargs='+', default=[0.0],
    ...
    parser.add_argument('--export_adapted_csv', type=str, default=None,
                        help='Optional path to write a human-readable adapted poker CSV (adds adapted_token column)')
    parser.add_argument('--export_only', action='store_true',
                        help='If set, only export adapted CSV (requires --export_adapted_csv) and exit')
    ...
    print("Loading data...")
    sequences, labels, symbol_to_idx, omega_idx, vocab_size, formula_data, alphabet, append_omega = load_data(
        args.csv_path, args.json_path, export_adapted_csv=args.export_adapted_csv
    )
    print(f"Loaded {len(sequences)} sequences")
    print(f"Vocabulary size: {vocab_size}")
    print(f"Alphabet size: {vocab_size - 1}")
    print(f"Append omega token: {append_omega}")

    if args.export_only:
        if args.export_adapted_csv:
            print(f"Exported adapted dataset to: {args.export_adapted_csv}")
        else:
            print("Nothing exported (missing --export_adapted_csv).")
        return
    ...
    if len(args.noise_types) == 1:
        ...
        print(f"{'Level':<10} {'Accuracy':<12} {'Precision':<12} {'Recall':<12} {'F1':<12} {'ROC-AUC':<12}")
        print("-"*80)
        for key in sorted(all_results.keys(), key=lambda x: all_results[x]['noise_level']):
            ...
            if r['noise_type'] == noise_type:
                ...
                print(f"{r['noise_level']:<10.2f} {r['accuracy']:<12.4f} {r['precision']:<12.4f} {r['recall']:<12.4f} {r['f1']:<12.4f} {roc_str:<12}")
    else:
        print(f"{'Type':<15} {'Level':<10} {'Accuracy':<12} {'Precision':<12} {'Recall':<12} {'F1':<12} {'ROC-AUC':<12}")
        print("-"*80)
        for key in sorted(all_results.keys(), key=lambda x: (all_results[x]['noise_type'], all_results[x]['noise_level'])):
            r = all_results[key]
            roc = r.get('roc_auc', None)
            roc_str = f"{roc:.4f}" if roc is not None else "n/a"
            print(f"{r['noise_type']:<15} {r['noise_level']:<10.2f} {r['accuracy']:<12.4f} {r['precision']:<12.4f} {r['recall']:<12.4f} {r['f1']:<12.4f} {roc_str:<12}")
    print("="*80)
