"""
Train Simplified Transformer with Hard Attention on temporal logic datasets.
...
class SimplifiedTransformer:
    """Simplified Transformer with hard attention."""
    ...
    def leftmost_argmax_row_wise(self, M):
        """Hard attention: leftmost argmax for each row."""
        n_rows, n_cols = M.shape
        A = np.zeros_like(M)
        for row_idx in range(n_rows):
            row_scores = M[row_idx]
            max_score = np.max(row_scores)
            max_indices = np.where(row_scores == max_score)[0]
            if len(max_indices) > 0:
                selected_col = max_indices[0]
                A[row_idx, selected_col] = 1.0
        return A
    
    def forward_hard(self, sequence: List[str], alphabet: List[str]) -> float:
    ...
    def predict_hard(self, sequence: List[str], alphabet: List[str]) -> int:
        """Predict with hard attention."""
        output = self.forward_hard(sequence, alphabet)
...
def train_transformer_hard(X_train: List[List[str]], y_train: List[int],
    ...
    for epoch in range(epochs):
        ...
        if perturbations > 10:
            ...
            model.W_enc += np.random.randn(*model.W_enc.shape) * perturbation_strength
            for i in range(model.L):
                model.Q_layers[i] += np.random.randn(*model.Q_layers[i].shape) * perturbation_strength
                model.K_layers[i] += np.random.randn(*model.K_layers[i].shape) * perturbation_strength
                model.V_layers[i] += np.random.randn(*model.V_layers[i].shape) * perturbation_strength
                model.O_layers[i] += np.random.randn(*model.O_layers[i].shape) * perturbation_strength
    ...
    y_pred = [model.predict_hard(seq, alphabet) for seq in X_test]
    y_pred_proba = [1.0 / (1.0 + np.exp(-model.forward_hard(seq, alphabet))) for seq in X_test]
...
def train_on_dataset(dataset_dir: str, formula_id: int = 33, output_dir: str = None, epochs: int = None):
    ...
    try:
        ...
        plt.plot(fpr, tpr, label=f"Transformer Hard (AUC={result['auc']:.3f})", linewidth=2.5, color='green')
        plt.plot([0, 1], [0, 1], 'k--', label='Random Classifier', linewidth=2)
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('False Positive Rate', fontsize=12)
        plt.ylabel('True Positive Rate', fontsize=12)
        plt.title(f'ROC Curve - Transformer Hard Attention ({dataset_dir})', fontsize=14, fontweight='bold')
        ...
        roc_path = os.path.join(output_dir, f'roc_curve_transformer_hard_{dataset_name}.png')
    ...
    results_path = os.path.join(output_dir, f'transformer_hard_results_{dataset_name}.json')
